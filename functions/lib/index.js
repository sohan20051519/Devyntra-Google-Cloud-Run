...
exports.deployWebhook = exports.deployStream = exports.deploy = void 0;
...
async function resolveFunctionUrl(projectId, region, functionName) {
    try {
        const token = await getAccessToken();
...
async function ensureSecret(projectId, name, value) {
    const token = await getAccessToken();
    const headers = { Authorization: `Bearer ${token}`, "Content-Type": "application/json" };
    // Create secret if missing
    try {
        await axios_1.default.get(`https://secretmanager.googleapis.com/v1/projects/${projectId}/secrets/${name}`, { headers });
    }
    catch {
        try {
            await axios_1.default.post(`https://secretmanager.googleapis.com/v1/projects/${projectId}/secrets?secretId=${name}`, { replication: { automatic: {} } }, { headers });
        }
        catch { }
    }
    // Add version
    try {
        ...
        await axios_1.default.post(`https://secretmanager.googleapis.com/v1/projects/${projectId}/secrets/${name}:addVersion`, { payload: { data: payload } }, { headers });
    }
    catch { }
}
// Ensure a workflow is enabled (it can be disabled due to inactivity or manually)
async function ensureWorkflowEnabled(octokit, owner, repo, workflowFile) {
    try {
        ...
        if (id) {
            try {
                ...
                const state = wf.data?.state?.toString().toLowerCase();
...
async function deleteFileIfExists(octokit, owner, repo, path, branch) {
    try {
        const existing = await octokit.repos.getContent({ owner, repo, path, ref: branch });
        // @ts-ignore
...
async function triggerWorkflowDispatch(octokit, owner, repo, workflowFile, refBranch, inputs) {
...
async function dispatchWithRetry(octokit, owner, repo, workflowFile, refBranch, inputs, retries = 10, delayMs = 7000) {
    let resolvedId = null;
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if ((status === 422 && (msg.includes("workflow does not have") || msg.includes("workflow_dispatch"))) ||
                status === 404 ||
...
async function resolveWorkflowId(octokit, owner, repo, workflowFile) {
    try {
...
async function waitForLatestWorkflowConclusion(octokit, owner, repo, workflowFile, timeoutMs = 120000, onProgress, shouldStop, sinceEpochMs) {
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            const cutoff = (sinceEpochMs ?? 0) - 10000; // allow small clock skew
            ...
            if (onProgress) {
                try {
                    ...
                    if (running && Array.isArray(running.steps)) {
                        const currentStep = running.steps.find((s) => s.status === 'in_progress') || running.steps.find((s) => s.status === 'queued') || running.steps.find((s) => s.conclusion === 'success') || running.steps?.[0];
...
exports.deploy = (0, https_1.onRequest)({
    ...
    try {
        ...
        if (req.body?.action === "redeploy") {
            ...
            if (!wrote)
                throw new Error("Failed to write SSE workflow onto any common branch (main/master/default). Check repo permissions.");
        ...
        if (req.body?.action === "rollback") {
            ...
            const list = await axios_1.default.get(listUrl, { headers });
            const revisions = list.data?.revisions || list.data?.items || [];
            ...
            await axios_1.default.patch(patchUrl, { traffic: [{ type: "TRAFFIC_TARGET_ALLOCATION_TYPE_REVISION", revision: previousRevision.split('/').pop(), percent: 100 }] }, { headers });
            const svc = await axios_1.default.get(patchUrl, { headers });
        ...
        if (req.body?.action === "delete_service") {
            ...
            try {
                ...
                const svc = await axios_1.default.get(getUrl, { headers });
            ...
            await axios_1.default.delete(delUrl, { headers });
        ...
        let analysisDispatchAt = undefined;
        if (!(await hasRecentRun(octokit, repoOwner, repoName, "analysis.yml"))) {
            ...
            if (!ok)
                throw new Error("GitHub did not accept workflow_dispatch for analysis.yml. Wait and retry.");
        }
        await setStatus("analyzing", "Running code analysis workflow...");
        await addLog("🔎 Running code analysis workflow...");
        let analysisConclusion = await waitForLatestWorkflowConclusion(octokit, repoOwner, repoName, "analysis.yml", 4 * 60000, async (info) => {
            const rawStep = info.stepName || info.jobName || "";
            const step = rawStep.toLowerCase();
            let sub = "pipeline";
            if (step.includes("build"))
                sub = "build_image";
            else if (step.includes("push"))
                sub = "push_image";
            else if (step.includes("deploy"))
                sub = "deploy";
            else if (step.includes("fetch service url"))
                sub = "fetch_url";
            ...
            try {
                if (rawStep)
                    await addLog(`➤ ${rawStep} — ${st || 'in_progress'}`);
            }
            catch { }
        }, async () => await isCancelled(), analysisDispatchAt);
        ...
        if (errorsFound) {
            ...
            while (errorsFound && julesUrl && julesKey && attempts < 2) {
                ...
                try {
                    ...
                    const julesResp = await axios_1.default.post(julesUrl, julesReq, { headers: { Authorization: `Bearer ${julesKey}` } });
                    ...
                    if (sessionUrl)
                        await addLog(`🔗 Jules session: ${sessionUrl}`);
                    if (prUrl)
                        await addLog(`🔀 PR created: ${prUrl}`);
                    ...
                    await addLog("🔁 Re-running analysis after Jules fixes...");
                    {
                        const ok = await dispatchWithRetry(octokit, repoOwner, repoName, ".github/workflows/analysis.yml", defaultBranch, { deployment_id: deploymentId });
                        if (!ok)
                            throw new Error("GitHub did not accept workflow_dispatch for analysis.yml after fixes.");
                    }
                    analysisConclusion = await waitForLatestWorkflowConclusion(octokit, repoOwner, repoName, "analysis.yml", 4 * 60000, undefined, async () => await isCancelled(), Date.now());
                    ...
                    logger.warn("Jules auto-fix pass failed", e);
        ...
        for (const br of branches) {
            try {
                await deleteFileIfExists(octokit, repoOwner, repoName, deployPath, br);
            }
            catch { }
            try {
                await createOrUpdateFile(octokit, {
        ...
        let deployDispatchAt = undefined;
        ...
        const deployConclusion = await waitForLatestWorkflowConclusion(octokit, repoOwner, repoName, "deploy-cloudrun.yml", 5 * 60000, async ({ jobName, stepName }) => {
            // Map common job names to substeps
            const name = (stepName || jobName || '').toLowerCase();
            let sub;
            if (name.includes('checkout') || name.includes('authenticate') || name.includes('set up gcloud') || name.includes('configure artifact'))
                sub = 'pipeline';
            else if (name.includes('install'))
                sub = 'install';
            else if (name.includes('build docker image') || name.includes('push docker image') || name.includes('docker'))
                sub = 'build_image';
            else if (name.includes('deploy to cloud run') || name === 'deploy')
                sub = 'deploy';
            if (sub) {
        ...
        }, async () => await isCancelled(), deployDispatchAt);
        if (await isCancelled()) {
            await setStatus("failed", "Deployment cancelled");
            await addLog("🛑 Deployment cancelled by client.");
            res.json({ deploymentId });
        ...
        if (deployConclusion === "success") {
            ...
            try {
                ...
                const { data } = await axios_1.default.get(svcUrl, { headers: { Authorization: `Bearer ${token}` } });
            ...
            if (deploymentUrl)
                await addLog(`🌐 Service URL: ${deploymentUrl}`);
            ...
            const chunkUntil = Date.now() + 3 * 60000; // up to ~3 more minutes within overall 9-min function timeout
            let concluded = null;
            while (Date.now() < chunkUntil) {
                concluded = await waitForLatestWorkflowConclusion(octokit, repoOwner, repoName, "deploy-cloudrun.yml", 60000, async ({ jobName, stepName }) => {
                    const name = (stepName || jobName || '').toLowerCase();
                    let sub;
                    if (name.includes('checkout') || name.includes('authenticate') || name.includes('set up gcloud') || name.includes('configure artifact'))
                        sub = 'pipeline';
                ...
                if (concluded === 'success' || concluded === 'failure')
                    break;
            }
            if (await isCancelled()) {
                await setStatus("failed", "Deployment cancelled");
...
exports.deployStream = (0, https_1.onRequest)({
    region: "us-central1",
    cors: true,
    ...
    try {
        ...
        const deploymentId = (req.query.deploymentId || req.query.id);
        if (!deploymentId) {
...
exports.deployWebhook = (0, https_1.onRequest)({
    region: "us-central1",
    cors: true,
    timeoutSeconds: 60,
}, async (req, res) => {
