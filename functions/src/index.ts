...
async function notifyProgress(webhookUrl: string | undefined, webhookToken: string | undefined, payload: Record<string, any>) {
  try {
    ...
    await axios.post(cleanUrl, payload, {
      headers: {
...
function autoRedeployWorkflowYml(defaultBranch: string, projectId: string, serviceName: string, region: string, fixedDeploymentId?: string): string {
  const depExpr = fixedDeploymentId ? fixedDeploymentId : '${{ github.run_id }}-${{ github.run_attempt }}';
  ...
  try {
    ...
    if (req.body?.action === "set_auto_redeploy") {
      ...
      if (!webhookUrl && projectId) {
        try { const maybe = await resolveFunctionUrl(projectId, "us-central1", "deployWebhook"); if (maybe) webhookUrl = maybe; } catch {}
      }
      ...
      if (!webhookToken && projectId) {
        try { webhookToken = crypto.randomBytes(24).toString('hex'); await ensureSecret(projectId, "DEVYNTRA_WEBHOOK_TOKEN", webhookToken); } catch {}
      }
      ...
      if (webhookUrl) await upsertRepoSecret(octokit, repoOwner, repoName, "DEVYNTRA_WEBHOOK_URL", (webhookUrl || '').trim());
      if (webhookToken) await upsertRepoSecret(octokit, repoOwner, repoName, "DEVYNTRA_WEBHOOK_TOKEN", (webhookToken || '').trim());
      ...
      if (enable) {
        await createOrUpdateFile(octokit, {
          ...
          content: autoRedeployWorkflowYml(defaultBranch, projectId || "", serviceName, region, deploymentIdParam || undefined),
      ...
      if (deploymentIdParam) {
        try { await db.collection("deployments").doc(deploymentIdParam).set({ autoRedeployEnabled: enable, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true }); } catch {}
      }
    ...
    let expectedWebhookToken: string | null = null;
    try {
    ...
    if (finalWebhookToken) await upsertRepoSecret(octokit, repoOwner, repoName, "DEVYNTRA_WEBHOOK_TOKEN", finalWebhookToken);
