...
async function triggerWorkflowDispatch(octokit: Octokit, owner: string, repo: string, workflowFile: string, refBranch: string) {
...
async function waitForLatestWorkflowConclusion(octokit: Octokit, owner: string, repo: string, workflowFile: string, timeoutMs = 120000): Promise<string | null> {
    ...
    if (errorsFound) {
      ...
      if (julesUrl && julesKey) {
        try {
          ...
          logger.warn("Jules API call failed", e as any);
        }
      }
    }
